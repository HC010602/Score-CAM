...
def apply_transforms(image, size=224):
    ...
    if not isinstance(image, Image.Image):  # 判断图像是否为PIL类型
        image = F.to_pil_image(image)       # 若不是则转为PIL图像

    means = [0.485, 0.456, 0.406]
    stds = [0.229, 0.224, 0.225]            # ImageNet数据集的均值和方差

    transform = transforms.Compose([        # 对图像依次进行下列操作
        ...
        transforms.ToTensor(),              # PIL.Image或ndarray从 (H x W x C)形状转换为 (N x C x H x W) 的tensor，便于pytorch处理
        transforms.Normalize(means, stds)   # 对图像按通道进行标准化，即减去均值，再除以方差
    ])

    tensor = transform(image).unsqueeze(0)  # 在位置0处增加一个维度(x,y)->(add,x,y)，用来存放batch

    tensor.requires_grad = True             # PyTorch的自动求导机制会对这个tensor以及由这个tensor计算出来的其他tensor求导，并将导数值存在tensor的grad属性中
...
def basic_visualize(input_, gradients, save_path=None, weight=None, cmap='viridis', alpha=0.7):
    ...
    """
    input_ = format_for_plotting(denormalize(input_))                 # 图片反归一化，resize (N, C, H, W) -> (H, W, C)
    gradients = format_for_plotting(standardize_and_clip(gradients))  # Score-CAM标准化后再归一化
    ...
    num_subplots = len(subplots)  # 2

    fig = plt.figure(figsize=(4, 4))  # 创建画板fig

    for i, (title, images) in enumerate(subplots):
        ax = fig.add_subplot(1, num_subplots, i + 1)   # 选择第i张画纸
        ax.set_axis_off()                              # 关闭xy轴

        for image, cmap, alpha in images:
            ax.imshow(image, cmap=cmap, alpha=alpha)   # 绘图  cmap='viridis', alpha=0.7
