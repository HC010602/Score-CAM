...
class BaseCAM(object):
    ...
    def __init__(self, model_dict):
        ...
        layer_name = model_dict['layer_name']           # 为什么要分 类属性和实例属性？
        
        self.model_arch = model_dict['arch']            # 存放模型的预训练权重
        self.model_arch.eval()
        if torch.cuda.is_available():
          self.model_arch.cuda()
        self.gradients = dict()                         # 初始化空字典对象
        ...
        def forward_hook(module, input, output):            # 定义前向传播的hook，记录下该层输出的激活图
            if torch.cuda.is_available():
        ...
        self.target_layer.register_forward_hook(forward_hook)      # 为target_layer注册forward_hook，每次执行forward到目标层时都会调用该方法
